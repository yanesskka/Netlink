...
TEST (capture_start) 
{
    ...
    // strating tcpdump execution with writing to the file
...
TEST (incorrect_family_id) 
{
    ...
    user_hdr = genlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, family_id + 1, 0, NLM_F_REQUEST, MY_GENL_SMALL_CMD_GET_NESTED, 0);
    if (!user_hdr) {
        fprintf(stderr, "Failed to genlmsg_put\n");
        nlmsg_free(msg);
        nl_socket_free(sock);
...
TEST (incorrect_genl_third_echo_flags) 
{
    ...
    err = nl_recvmsgs(sock, cb);
    if (err < 0) {
        EXPECT_EQ(err, -NLE_INVAL);    // invalid flags were sent
    }
...
TEST (incorrect_parallel_genl_reject_policy_set_value) 
{
    ...
    err = nl_recvmsgs(nl_sock, cb);
    if (err < 0) {
        EXPECT_EQ(err, -NLE_INVAL);   // reject policy - all attributes were skipped  (in my code I send -EINVAL)
    }
